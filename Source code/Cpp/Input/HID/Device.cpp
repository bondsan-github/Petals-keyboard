...
namespace HID
{
    Device::Device( HANDLE device, Application & application )
    : application( application ), Collections( application )
    {
    ...
    std::wstring Device::path()
    {
        ...
        return_value = GetRawInputDeviceInfoW( raw_handle, request.path, path_.data() , & path_char_amount );
